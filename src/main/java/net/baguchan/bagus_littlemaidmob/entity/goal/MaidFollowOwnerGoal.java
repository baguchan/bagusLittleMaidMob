...
import net.baguchan.bagus_littlemaidmob.entity.LittleMaid;
...
public class MaidFollowOwnerGoal extends Goal {
	public static final int TELEPORT_WHEN_DISTANCE_IS = 12;
	private static final int MIN_HORIZONTAL_DISTANCE_FROM_PLAYER_WHEN_TELEPORTING = 2;
	private static final int MAX_HORIZONTAL_DISTANCE_FROM_PLAYER_WHEN_TELEPORTING = 3;
	private static final int MAX_VERTICAL_DISTANCE_FROM_PLAYER_WHEN_TELEPORTING = 1;
	private final LittleMaid tamable;
	...
	public MaidFollowOwnerGoal(LittleMaid p_25294_, double p_25295_, float p_25296_, float p_25297_, boolean p_25298_) {
	...
	public boolean canContinueToUse() {
		if (this.navigation.isDone()) {
			return false;
		} else if (this.tamable.isOrderedToSit() || LittleMaidBaseEntity.MoveState.get(this.tamable.getMovingState()) == LittleMaidBaseEntity.MoveState.FREEDOM) {
			return false;
		} else {
	...
	public void tick() {
		...
		if (--this.timeToRecalcPath <= 0) {
			...
			if (!this.tamable.isLeashed() && !this.tamable.isPassenger()) {
				if (this.tamable.distanceToSqr(this.owner) >= 180.0D) {
